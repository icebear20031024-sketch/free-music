...
### 阶段 9: 解决部分源（酷狗/酷我/咪咕等）跨域/防盗链导致图片封面损坏报错 (E2E 验证)
- [x] 审计各插件返回的图片源路径，发现大多数音源网站 CDN 配置了严格的 Referral 校验以及 CORS/混合内容 Block 策略
- [x] 修改前端 React `<img>` 标签添加 `referrerPolicy="no-referrer"` 控制标头
- [x] 引入服务器端智能代理逻辑：在 `api.ts` 中实现通用的 `getProxiedCoverUrl` 动态图像代理机制，所有外链歌曲、歌手和歌单封面自动利用已有健全的 `/api/proxy?url=...` 控制头绕过机制经后端安全获取
- [x] 在所有播放器及封面渲染页（如 `PlayerBar.tsx`, `LyricsView.tsx`, `App.tsx`）增加 `onError` 降级容错，发生图片加载失败时智能回退到标准高画质 Unsplash 歌曲占位，确保音乐画幅完美呈现
